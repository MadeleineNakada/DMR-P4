...
class Learner(object):
    ...
    def reduce_state(self, state):
        ...
        if tree_dist_x > 300:
            ...
            tree_dist_x = int(tree_dist_x/30)
        # 2
        tree_top_y = int((state['tree']['top'] - state['monkey']['top']) / 20)
        # 3
        tree_bottom_y = int((state['tree']['bot'] - state['monkey']['bot']) / 20)
        # 4
        vel = int(state['monkey']['vel']/15)
        return (tree_dist_x, tree_top_y, tree_bottom_y, state['monkey']['top'], state['monkey']['bot'], vel)
    ...
    def reward_callback(self, reward):
        ...
        # print('x', last_state['monkey'])

        penalty = 0
        # if last_state['monkey']['bot'] < 50:
        #     # print('bb')
        #     penalty =  -100 * (50 - last_state['monkey']['bot'])
        # elif last_state['monkey']['top'] > 350:
        #     # print('tt')
        #     penalty =  -100 * (last_state['monkey']['top'] - 350)
        ...
        # print(vel)
        self.last_reward =  500 - abs(monkey_avg - (tree_avg - 100)) + penalty - abs(last_state['monkey']['vel'])
