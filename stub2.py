...
class Learner(object):
    ...
    def reduce_state(self, state):
        ...
        # 4
        vel = int(state['monkey']['vel'] / 2)
        d = 1 if vel >= 0 else -1
        # print(vel)
        # return (tree_dist_x, tree_top_y, tree_bottom_y, vel)
        return(state['monkey']['top'], state['monkey']['bot'], tree_top_y, tree_bottom_y, state['monkey']['vel'], tree_dist_x)
    ...
    def update_q_table(self, reduced_state):
        w = self.q[reduced_state]
        self.q[reduced_state] = 

    def basis(self, state, w):
        transformed = np.array([lambda s: np.exp(- 1 * (self.epsilon * np.sqrt(np.dot((state - s), (state - s)))) ** 2)])
        return np.dot(w, transformed)
        # transformed = []
        # for s in state:
    ...
    def action_callback(self, state):
        ...
        self.update_q_table(reduce_state)
    ...
    def reward_callback(self, reward):
        ...
        penalty = 0
        if last_state['monkey']['top'] > last_state['tree']['top']: 
            penalty = -10 * top_dist
        elif last_state['monkey']['bot'] < last_state['tree']['bot']:
            penalty = -10 * bot_dist
        # # vel = last_state['monkey']['vel']
        # # print(vel)
        # self.last_reward = (400 - last_state['tree']['dist'])(last_state['monkey']['bot'] - last_state['tree']['top'])
        self.last_reward =  (400 - max([top_dist, bot_dist])) + penalty
